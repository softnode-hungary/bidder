...
export default (doc, printData, startY, fontSize, lineSpacing) => {
    ...
    items.map(item => {
        ...
        doc.text(item.qty.toString(), tablecol2X, startY, 'right');

        doc.text(item.amount.toLocaleString('hu') + " Ft", tablecol3X, startY, 'right');

        doc.text(item.total.toLocaleString('hu') + " Ft", endX, startY, 'right');
